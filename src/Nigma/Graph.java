...
public class Graph extends JPanel {
    ...
    public void autoScale() {// автомаштабирование
    ...
    public void paintD(Graphics g) {// Отрисовка координатной плоскости
    ...
    public void paintArea(Graphics g) {
        // Устанавливаем цвет закраски
        g.setColor(new Color(255, 255, 0, 100)); // Желтый с непрозрачностью 100

        // Проходим по каждой точке и закрашиваем прямоугольник между графиками
        for (int i = 0; i < fun.Y1.size() - 1; i++) {
            int gx1 = (int) (-(x1 - step_x * fun.X.get(i)) / (x2 - x1) * WIDTH);
            int gx2 = (int) (-(x1 - step_x * fun.X.get(i + 1)) / (x2 - x1) * WIDTH);
            int q1 = HEIGHT - (int) Math.floor((HEIGHT / (Math.abs(y2 - y1))) * (fun.Y1.get(i) - y1));
            int q2 = HEIGHT - (int) Math.floor((HEIGHT / (Math.abs(y2 - y1))) * (fun.Y2.get(i) - y1));
            int q3 = HEIGHT - (int) Math.floor((HEIGHT / (Math.abs(y2 - y1))) * (fun.Y1.get(i + 1) - y1));
            int q4 = HEIGHT - (int) Math.floor((HEIGHT / (Math.abs(y2 - y1))) * (fun.Y2.get(i + 1) - y1));

            int[] xPoints = { gx1, gx2, gx2, gx1 }; // X координаты вершин
            int[] yPoints = { q1, q3, q4, q2 }; // Y координаты вершин

            // Отрисовываем закрашенный прямоугольник
